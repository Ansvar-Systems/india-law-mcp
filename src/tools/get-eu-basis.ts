/**
 * get_eu_basis — Retrieve EU/international legal basis for an Indian statute.
 *
 * For India, this provides cross-references to EU instruments that Indian laws
 * are modelled on or equivalent to (e.g., DPDPA → GDPR).
 */
...
export async function getEUBasis(
  ...
  const euDocuments: EUBasisDocument[] = rows.map(row => {
    const result: EUBasisDocument = {
      ...
      community: row.community as EUBasisDocument['community'], reference_type: row.reference_type as EUBasisDocument['reference_type'],
